...
arduinoBodyName =  '/dev/ardBody' #'/dev/ardBody'
aBNbaudrate = 115200
play = 0
file = 0
try:
...
def readlineCR(port):
    if port !="":
        ...
        return ""
statPlay = False
pipeAUDIO = subprocess.Popen(command, stdin=subprocess.PIPE)
def playerBOT(play):
    global statPlay
    global pipeAUDIO
    if play and not statPlay:
        pipeAUDIO = subprocess.Popen(command, stdin=subprocess.PIPE)
        pipeAUDIO.stdin.write(u"play\n")
        statPlay = True
    elif not play and statPlay:
        try:
            pipeAUDIO.kill()
        except:
            print 'error kill pipe Audio'
        statPlay = False
